import type { InferSelectModel } from "@nilovon-starterkit/db";
import {
  and,
  apikey,
  apikeyAccess,
  ...
  eq,
  isNull,
} from "@nilovon-starterkit/db";
import { cacheable } from "@nilovon-starterkit/redis";
...
export async function getApiKeyFromHeader(
  ...
  const bearer = auth?.toLowerCase()?.startsWith(BEARER_PREFIX)
    ? auth.slice(BEARER_PREFIX.length).trim()
    : null;
  const secret =
